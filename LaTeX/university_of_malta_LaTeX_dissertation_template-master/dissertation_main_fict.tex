...
\title{Galaxy Morphological Classification}  % use \\ here otherwise you get a justified title
                                     % note capitalization of the title (only common 
                                     % words in lower case)
\tagline{some hyped-up tag line}      % tag line
\author{Robert Mifsud}            % your full name
\authorID{137402L}                   % your University Identifier
\supervisor{Mr Joseph Bonello}              % your supervisor(s) name - no . in Dr
% \cosupervisor{}                % your cosupervisor(s) name - no . in Dr *OPTIONAL* 
                                     % simply comment out the above line if absent

\degreeName{B.Sc. Information Technology (Software Development)}       		 % the degree you are reading
                                     % note the \ after the dot, so not to consider it a fullstop
\doctype{fyp}               % the type of document (fyp, dissertation, thesis)
...
\graphicspath{{./images/}{./chap1/images/}{./chap2/images/}{./appGalaxies/images/}}   % Paths where to look for images, if defined "images" must always be there as it holds the images in-use by the template.
...
%    \input{frontmatter/copyright}     
    \includepdf[pages=-]{official_docs/authenticityformug.pdf}
...
{%backmatter % comment this out otherwise are not numbered
    ...
    \bibliographystyle{IEEEtran} % um.cls line: 38 commented to remove 'authoryear' package.
    ...
    % \nocite{*} %Include sources in bibliography which are not cited once
    % {\scriptsize\bibliography{chap2/fyp-bibliography}}
    {\scriptsize\bibliography{fyp_biblio}}

	\printindex
}

\appendix
    \input{appGalaxies/appendix_galaxies_main}
